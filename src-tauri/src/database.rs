...
// pub fn select_config1(db: &Connection, sql: &str) -> Result<collected_config, rusqlite::Error> {
//     let mut stmt = db.prepare(sql)?;
//     let rows = stmt.query_map([], |row| {
//         let folderpath: String = row.get(1)?;
//         let pictureurl: String = row.get(2)?;
//         Ok(collected_config {
//           folderpath,
//           pictureurl
//         })
...
// pub fn select_config(db: &Connection, sql: &str) -> Result<Vec<PictronConfig>, rusqlite::Error> {
//   let mut stmt = db.prepare(sql)?;
//   let rows = stmt.query_map([], |row| {
//       // let srcs_str: String = row.get(3)?;
//       // let srcs: Vec<ImgDetail> = from_str(&srcs_str).unwrap_or_default();
//       Ok(PictronConfig {
//           folderpath: row.get(1)?,
//           pictureurl: row.get(2)?,
       
//       })
//   })?;

//   let mut collected_config = Vec::new();
//   for row in rows {
//       collected_config.push(row?);
//   }
